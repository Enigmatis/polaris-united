...
export class IrrelevantEntitiesMiddleware {
  ...
  private static async queryIrrelevant(connection: PolarisConnection, typeName: string, context: PolarisGraphQLContext, result: any) {
    ...
    return async (
      ...
      if (
        ...
        if (connection.hasRepository(typeName)) {
          ...
          const resultIrrelevant = await IrrelevantEntitiesMiddleware.queryIrrelevant(connection, typeName, context, result);
