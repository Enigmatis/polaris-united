...
export class TypeOrmCoreModule implements OnApplicationShutdown {
    ...
    static forRoot(options: PolarisTypeOrmModuleOptions): DynamicModule {
        ...
        const connectionProvider = {
            ...
            useFactory: async () =>
                this.createConnectionFactory(options, options.connectionManager, options.logger),
        };
        ...
        return {
            ...
            providers: [entityManagerProvider, connectionProvider, typeOrmModuleOptions],
    ...
    static forRootAsync(options: TypeOrmModuleAsyncOptions): DynamicModule {
        ...
        const entityManagerProvider = {
            provide: getEntityManagerToken((options as unknown) as ConnectionOptions) as string,
    ...
    private static createAsyncOptionsProvider(options: TypeOrmModuleAsyncOptions): Provider {
        ...
        const inject = [(options.useClass || options.useExisting) as Type<TypeOrmOptionsFactory>];
        return {
            provide: TYPEORM_MODULE_OPTIONS,
            useFactory: async (optionsFactory: TypeOrmOptionsFactory) =>
                optionsFactory.createTypeOrmOptions(options.name),
            inject,
        };
    }

    private static createEntityManagerProvider(options: ConnectionOptions): Provider {
        return {
    ...
    private static async createConnectionFactory(
        ...
        return defer(() => {
            if (!options.type) {
                return createPolarisConnection(options as ConnectionOptions, polarisLogger as any);
            }
            ...
            if (entities) {
                ...
                entities = EntitiesMetadataStorage.getEntitiesByConnection(connectionToken);
            }
            const connectionOptions: any = {
                ...options,
                entities,
            };
            return createPolarisConnection(connectionOptions, polarisLogger as any);
        })
